...
void PairNNMP::compute(int eflag, int vflag)
{
	...
	if (newton_pair==1)
		error->all(FLERR, "Pair style nnmp requires 'newton off'");
	*/
...
//////////////////////////////////////////////////////////////////////////////////////
// set forces for ghost atoms and apply virial
//////////////////////////////////////////////////////////////////////////////////////
void PairNNMP::computeVirial()
{
	int verbose=0;
	if(list==NULL) return;
	double fall[] = {0,0,0};
	...
	{
		cout<<"box="<<endl;
		cout<<scientific<<setprecision(6)<<setw(m)<<box[0][0]<<" "<<setw(m)<<box[0][1]<<" "<<setw(m)<<box[0][2]<<endl;
		cout<<scientific<<setprecision(6)<<setw(m)<<box[1][0]<<" "<<setw(m)<<box[1][1]<<" "<<setw(m)<<box[1][2]<<endl;
		cout<<scientific<<setprecision(6)<<setw(m)<<box[2][0]<<" "<<setw(m)<<box[2][1]<<" "<<setw(m)<<box[2][2]<<endl;
		cout<<endl;
		for (int j = 0; j<atom->nlocal; j++) {
			int type = atom->type[j]-1;
			...
			cout<<setw(5)<<elements[type].c_str()<<" "<<
			scientific<<setprecision(6)<<setw(m)<< atom->x[j][0]
			<<" "<<setw(m)<<atom->x[j][1]
			<<" "<<setw(m)<<atom->x[j][2]
			<<" "<<setw(m)<<atom->f[j][0]
			<<" "<<setw(m)<<atom->f[j][1]
			<<" "<<setw(m)<<atom->f[j][2]<<endl;
		}
		for (int j = 0; j<atom->nlocal; j++)
			for(int c=0;c<3;c++)
				fall[c] += atom->f[j][c];
		cout<<"fall="<<fall[0]<<" "<<fall[1]<<" "<<fall[2]<<endl;
	}
	int nall = atom->nlocal + atom->nghost;
	// setForces for fdotr
	vector<int> ng(atom->nlocal,0);
	for (int j = 0; j < nall; j++){
		int tj=atom->tag[j]-1;
		int i=tag2i[tj];
		ng[i]++;
	}
	for (int j = atom->nlocal; j < nall; j++){
		int tj=atom->tag[j]-1;
		int i=tag2i[tj];
		for(int c=0;c<3;c++)
			atom->f[j][c] = atom->f[i][c]/ng[i];
	}
	for (int j = 0; j < atom->nlocal; j++){
		for(int c=0;c<3;c++)
			atom->f[j][c] = atom->f[j][c]/ng[j];
	}
	for(int c=0;c<3;c++)
		fall[c] = 0;
	for (int j = 0; j < nall; j++){
	...
	if(verbose>0)
	{
		cout<<"fallall="<<fall[0]<<" "<<fall[1]<<" "<<fall[2]<<endl;
		cout<<"Call virial_fdotr_compute"<<endl;
	}
...
void PairNNMP::computeEnergyAndForces()
{
	double energy;
	int i;
	if (pComputeEnergyAndForces &&  PyCallable_Check(pComputeEnergyAndForces))
	{
		int nAtoms= pos.size();
