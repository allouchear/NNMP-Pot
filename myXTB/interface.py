...
class Calculator(Molecule):
	...
	filename=None,
        ...
        """Create new calculator object (modified by Allouche, filename parameter added)"""
        Molecule.__init__(
            self, numbers, positions, charge, uhf, lattice, periodic,
        )
        self._calc = new_calculator()
        self._load(param,filename)

    def _load(self, param: Param, filename=_ffi.NULL):
        """Load parametrisation data into calculator (modified by Allouche, filename parameter added)"""

        if filename is not None:
                _filename = _ffi.new("char[]", filename.encode())
                self._loader[param](self._env, self._mol, self._calc, _filename)
        else:
