...
class Ewald(Layer):
	...
	def _set_kvecs(self, kmax):
		""" Set integer reciprocal space cutoff for Ewald summation """
		k = []
		for i in range(3):
			kk = tf.range(kmax[i] + 1,dtype=self.dtype)
	...
	def _reciprocal_space( self, Qa, R, Cell, idx_i, batch_seg, eps: float = 1e-8):
		...
		#print('Qashape=',Qa)
		#print('R=',R)
		QaR= Qa*tf.transpose(R)
		QaR= tf.transpose(QaR)
		QaR= tf.math.segment_sum(QaR, batch_seg)
		#print("QaR*QaR=", QaR*QaR)
		e_dipolar = prefactor/3.0 * tf.math.reduce_sum(QaR*QaR)
		#print('e_dipolar=',e_dipolar)
		#print('max e_dipolar=',tf.reduce_max(e_dipolar))
		#print('Qmol=',tf.math.segment_sum(Qa, batch_seg))
		#print('e_dipolar+eself=',e_dipolar+e_self)
		e_charge = -prefactor/(4*self._alpha2) * tf.math.reduce_sum(tf.math.segment_sum(Qa, batch_seg))
		#print("echarge", e_charge)

		# e_reciprocal by molecule
		e_reciprocal = (e_reciprocal + e_self + e_dipolar + e_charge)
