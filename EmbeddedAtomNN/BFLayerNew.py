...
class BFLayer(Layer):
	...
	def __init__(self, K, cutoff, Lmax=2, beta=0.2, name=None, basis_type="Gaussian", dtype=tf.float32):
		...
		if self.basis_type=="Gaussian":
			...
			n=tf.linspace(tf.constant(1.0,dtype=dtype),K)
	...
	def radial(self, rij):
		if self.basis_type=="Gaussian":
			...
			v = self.normc*sin(self.alphas*rij)/rij #  From Bessel
			v *= self.cutoff_fncos(rij)
			return v;
