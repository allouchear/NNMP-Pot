...
class BFLayer(Layer):
	...
	def __init__(self, K, cutoff, Lmax=2, beta=0.2, name=None, basis_type="Default", dtype=tf.float32):
		...
		#coefs = coefs*1e-2
		#coefs = tf.random.uniform([95,95,len(L),K], minval=-1e-3, maxval=1e-3, seed=0, dtype=dtype)
		coefs = tf.random.normal([95,95,len(L),K], mean=0.0, stddev=1.0e-2, seed=0, dtype=dtype)
		#coefs = np.zeros((95,95,len(L),K))
		#print("coefs=",coefs)
	...
	def __call__(self, Z, R, idx_i, idx_j, offsets=None):
		bfred, rij = self._computeEABF(Z, R, idx_i, idx_j, offsets=offsets)
		return bfred,rij

	def getPairs(self, Z, R, idx_i, idx_j, offsets=None):
