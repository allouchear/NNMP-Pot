...
class EmbeddedAtomMessagePassingNeuralNetwork(Layer):
	def __str__(self):
		return "Embedded Atom Message Passing Neural Network"+str(self.bf_layer.basis_type)+" radial basis"
	...
	def __init__(self,
		...
		seed=None):
		super().__init__(dtype=dtype, name="EmbeddedAtomMessagePassingNeuralNetwork")
		...
		self._bf_layer = BFLayer(K,  self._sr_cut, beta=beta, basis_type=basis_type, name="bf_layer",dtype=dtype) # size : K*K
	...
	def atomic_properties(self, Z, R, idx_i, idx_j, M=None, QaAlpha=None, QaBeta=None, offsets=None, sr_idx_i=None, sr_idx_j=None, sr_offsets=None, batch_seg=None):
		...
		outputs, nhloss = self.nn_outputs(x, bf, idx_i, idx_j)
