...
class PhysModelEEM(tf.keras.Model):
	def __str__(self):
	...
	def __init__(self,
		...
		seed=None):
		super().__init__(dtype=dtype, name="PhysModelEEM")
		...
		if ((self.use_electrostatic and (self.energy_weight > 0 or self.force_weight > 0))) or (self.charge_weight>0) or (self.atomic_charge_weight) or (self.dipole_weight):
			self._electrostatic_model = Electrostatic(model_type="EEM",
				sr_cut=sr_cut, lr_cut=lr_cut, eem_fit_parameters=eem_fit_parameters,
				use_scaled_charges=False, 	dtype=dtype) # already scaled
	...
	def computeAtomicEnergiesAndForces(self, data):
		print("====================== Error ==============================");
		print("computeProperties not yet implemented in PhysModelEEM.py");
		print("====================== Error ==============================");
		sys.exit()
	...
	def computeProperties(self, data):
		...
		with tf.GradientTape() as g:
			...
			Ea = outputs[:,0]
			#print("Ea=",Ea)
			#Xia = tf.math.abs(tf.constant(outputs[:,1]))
			Xia = (tf.constant(outputs[:,1]))
			#print("Xiaabs0=",tf.math.abs(Xia))
			#Xiaabs=tf.math.abs(Xia)
			#Xia=-Xiaabs
			#print("Xiaabs1=",Xia)
			#Xia = outputs[:,1]
			charges=None
			...
			if self.use_electrostatic and (self.energy_weight > 0 or self.force_weight > 0):
				Elec,QaScaled = self._electrostatic_model.energy_per_atom(Z=Z, R=R, Xia=Xia, Q_tot=data['Q'], batch_seg=data['batch_seg'])
				...
				if self._electrostatic_model is not None:
					Qa = self._electrostatic_model.compute_atomic_charges(Z=Z, R=R, Xia=Xia, Q_tot=data['Q'], batch_seg=data['batch_seg'])
				else:
					Qa=None
