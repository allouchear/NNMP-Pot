...
class PhysModelGFN1(tf.keras.Model):
	def __str__(self):
		...
		st += "\nGFN1 Electrostatic energy, NN charges"
	...
	def __init__(self,
		...
		gfn_fit_parameters=0,		  # fit gfn parameters (0 no, 1 fit J, 2 fit alp, 3 fit J & alp
		...
		seed=None):
		super().__init__(dtype=dtype, name="PhysModelGFN1")
		...
		if ((self.use_electrostatic and (self.energy_weight > 0 or self.force_weight > 0))) or (self.charge_weight>0) or (self.atomic_charge_weight) or (self.dipole_weight):
			self._electrostatic_model = Electrostatic(model_type="GFN1",  
				sr_cut=sr_cut, lr_cut=lr_cut, orbfile=orbfile, gfn_fit_parameters=gfn_fit_parameters,
				use_scaled_charges=use_scaled_charges, 	dtype=dtype)

		num_outputs=1
		if self._electrostatic_model is not None:
			num_outputs= self._electrostatic_model.num_outputs
	...
	def computeAtomicEnergiesAndForces(self, data):
		print("====================== Error ==============================");
		print("computeProperties not yet implemented in PhysModelGFN1.py");
		print("====================== Error ==============================");
		sys.exit()
	...
	def computeProperties(self, data):
		...
		with tf.GradientTape() as g:
			...
			Qa = self._electrostatic_model.compute_atomic_charges(Z, R, Qa=outputs[:,1:-1])
			...
			if self.use_electrostatic and (self.energy_weight > 0 or self.force_weight > 0):
				Eae , QaScaled =self._electrostatic_model.energy_per_atom(Z,R, Dij, Qa=outputs[:,1:-1], idx_i=idx_i, idx_j=idx_j, Q_tot=data['Q'], batch_seg=data['batch_seg'])
