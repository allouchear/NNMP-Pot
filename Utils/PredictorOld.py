...
def get_data_from_atoms(conv_distance, atoms, use_neighborlist, lr_cutoff, sr_cutoff):
		#find neighbors and offsets
		if use_neighborlist or any(atoms.get_pbc()):
		...
		# build data from Z,R,idx_i,idx_j,batch_seg
		data = {}
		...
		if offsets is not None:
			data['offsets'] =   offsets*conv_distance
		data['sr_offsets'] =   sr_offsets
		if sr_offsets is not None:
			data['sr_offsets'] =   sr_offsets*conv_distance
		data['sr_idx_i'] =  sr_idx_i
		data['sr_idx_j'] =  sr_idx_j
		data['M'] =  atoms.get_masses()
		data['QaAlpha'] =  None
		data['QaBeta'] =  None
		#print("R=",data['R'])
		#print("idxlen=",len(data['idx_i']))
